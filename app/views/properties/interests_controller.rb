  ...
  def create
    ...
    if user_signed_in?
      ...
      if Interest.exists?(property_id: @property.id, user_id: current_user.id)
        redirect_to request.referer.presence || project_property_path(@property.project, @property),
    ...
    if @interest.save
      redirect_to request.referer.presence || project_property_path(@property.project, @property),
                  notice: "Thank you! Your interest has been recorded. The owner will contact you soon."
    else
      redirect_to request.referer.presence || project_property_path(@property.project, @property),
