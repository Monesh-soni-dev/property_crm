...
class PropertiesController < ApplicationController
  ...
  def index
    ...
    # Preload interest data — avoids N+1 queries on the property card loop
    if @properties.any?
      property_ids = @properties.map(&:id)
      if user_signed_in? && current_user.customer?
        @user_interested_ids = Interest
          .where(user_id: current_user.id, property_id: property_ids)
          .pluck(:property_id).to_set
      else
